...
covid_model = LinearRegression().fit(df.drop(columns="new_cases"), df["new_cases"])

# Make predictions on the test set
covid_pred = covid_model.predict(df.drop(columns="new_cases"))

# Evaluate the model
mse = mean_squared_error(df["new_cases"], covid_pred)
print(f'Mean Squared Error: {mse}')

# Visualize the results
plt.scatter(df.drop(columns="new_cases"), df["new_cases"], color='black', label='Actual')
plt.plot(df.drop(columns="new_cases"), covid_pred, color='blue', linewidth=3, label='Predicted')
...
connect_server = RSConnectServer(
    url=os.getenv("CONNECT_SERVER"), # load from an .env file
    api_key=os.getenv("CONNECT_API_KEY") # load from an .env file 
)
